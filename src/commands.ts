...
export class Commands {
    ...
    public async enableCmakeTestCoverage() {
        this._assertWorkspaceNotEmpty("Cannot enable CMake test coverage on an empty workspace");
        const ws = await this.showWorkspacePicker();
        ...
        try {
            ...
            const rbfileContents = [
                ...
                '        CMAKE_CXX_FLAGS',
                '    ].each do |key|',
                '        existing = pkg.all_defines[key].to_s',
                '        unless existing.include?("--coverage")',
                '            pkg.define key, "#{existing} --coverage".strip',
                '        end',
                '    end',
                'end',
                ''
            ...
            await fs.mkdir_p(overridesPath);
            await fs.writeFile(path.join(overridesPath, "vscode-autoproj-cmake-coverage.rb"), rbfileContents);

            const config = vscode.workspace.getConfiguration("autoproj")
            const supressNotice = config.get<boolean>("supressCmakeCoverageOverrideNotice");

            if (!supressNotice) {
                const message = [
                    "A vscode-autoproj-cmake-coverage.rb file was created in 'autoproj/overrides.d' ",
                    `on the '${ws.name}' workspace. You can revert this operation by simply removing `,
                    'that file. Rebuild for changes to take effect.'
                ]
                ...
                if (item && !item.isCloseAffordance) {
                    config.update("supressCmakeCoverageOverrideNotice", true);
                }
            }
        } catch (error) {
            throw new Error(`Could not create overrides script: ${error.message}`);
    ...
    public register(subscriptions: vscode.Disposable[]) {
        ...
        subscriptions.push(vscode.commands.registerCommand("autoproj.enableCmakeTestCoverage", () => {
            this.handleError(() => this.enableCmakeTestCoverage());
        }));
